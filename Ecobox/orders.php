...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'submit_review') {
    ...
    if (mysqli_query($link, $sql_insert_review)) {
        ...
        // ── 寫入商家的評論通知 ──
        $res_store = mysqli_query($link, "SELECT No FROM store WHERE store_name = '$r_store_name' LIMIT 1");
        $row_store = mysqli_fetch_assoc($res_store);

        if ($row_store) {
            $actual_store_id  = (int)$row_store['No'];
            $notify_content   = mysqli_real_escape_string($link, "消費者「{$uName}」對您的店鋪留下了新評論（{$r_rating} 顆星）：{$_POST['review_comment']}");
            mysqli_query($link, "INSERT INTO seller_notifications (store_id, type, content, is_read, created_at)
...
$sql_orders = "SELECT o.*, s.No AS store_no 
               ...
               WHERE o.user_name = '$uName_escaped'
               ORDER BY o.No DESC";
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .expand-review-panel { display: none; margin-top: 5px; padding-top: 18px; border-top: 1px dashed #dddddd; align-items: flex-start; gap: 16px; }
                        ...
                        <div class="order-block-unit">
                            ...
                            <form method="POST" action="orders.php">
                                ...
                                <div class="expand-review-panel" id="review_panel_<?php echo $u_idx; ?>">
                                    <div class="user-avatar-circle"><img src="image/user_1.png" alt="使用者頭像"></div>
    ...
    <script>
        ...
        function rateStars(panelIdx, scoreValue) {
            ...
            stars.forEach((star, index) => {
                star.className = index < scoreValue ? 'fa-solid fa-star selected' : 'fa-regular fa-star';
            });
        }
        ...
        function appendEmoji(idx, emojiChar) {
            document.getElementById('comment_input_' + idx).value += emojiChar;
            document.getElementById('emoji_pop_' + idx).style.display = 'none';
            document.getElementById('comment_input_' + idx).focus();
        }
