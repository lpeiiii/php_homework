...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'clear_cart') {
    ...
    $final_price = isset($_POST['final_price']) ? (int)$_POST['final_price'] : 0;
    ...
    while ($cart_row = mysqli_fetch_assoc($res_get_cart)) {
        ...
        $sql_ins_order = "INSERT INTO orders_history (user_name, store_name, order_date, items_desc, total_price, status) 
                        VALUES ('$uName_escaped', '$s_name', CURDATE(), '$items_desc', $final_price, 1)";
        if (!mysqli_query($link, $sql_ins_order)) {
            echo json_encode([
                "status" => "error", 
                "message" => "【歷史訂單寫入失敗】錯誤原因: " . mysqli_error($link)
            ]);
            exit();
        }
        ...
        // 2. 扣減庫存
        $sql_reduce = "UPDATE seller_product 
                       SET quantity = quantity - $qty 
                       WHERE store_name = '$s_name' 
                       AND product_name = '$p_name' 
                       AND quantity >= $qty";
        mysqli_query($link, $sql_reduce);

        // 3. 扣完後檢查庫存是否剩 1，通知有收藏的消費者
        $sql_check = "SELECT quantity FROM seller_product 
                      WHERE store_name = '$s_name' AND product_name = '$p_name' LIMIT 1";
        $res_check = mysqli_query($link, $sql_check);
        $row_check = mysqli_fetch_assoc($res_check);

        if ($row_check && (int)$row_check['quantity'] === 1) {
            $sql_fav = "SELECT user_name FROM user_favorites WHERE store_name = '$s_name'";
            $res_fav = mysqli_query($link, $sql_fav);

            while ($fav_row = mysqli_fetch_assoc($res_fav)) {
                $fav_user    = mysqli_real_escape_string($link, $fav_row['user_name']);
                $fav_content = mysqli_real_escape_string($link, "您收藏的「{$cart_row['store_name']}」剩食盲盒僅剩最後 1 份，快去搶購！");
                mysqli_query($link, "INSERT INTO notifications (user_name, type, content, created_at) 
                                     VALUES ('$fav_user', 'order_ready', '$fav_content', NOW())");
            }
        }

        // 4. 寫入商家通知
        $sql_seller = "SELECT No, email FROM store WHERE store_name = '$s_name' LIMIT 1"; 
        ...
        if ($res_seller && $row_seller = mysqli_fetch_assoc($res_seller)) {
            ...
            $actual_store_id = (int)$row_seller['No'];
        }

        $notify_content = "收到來自 {$uName_escaped} 的新訂單：" . stripslashes($items_desc) . "，請著手準備！";


        if ($actual_store_id > 0) {
            $sql_notify = "INSERT INTO seller_notifications (store_id, type, content, is_read, order_id, created_at) 
                   VALUES ($actual_store_id, 'order', '$notify_content', 0, $new_order_id, NOW())";
            mysqli_query($link, $sql_notify);
        }
...
<body>
    ...
    <script>
        ...
        function clearCartAndGoHome() {
            ...
            const finalPriceStr = document.getElementById('receiptFinalPrice').innerText;
            formData.append('final_price', finalPriceStr);
