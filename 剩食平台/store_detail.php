...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['buy_store_name'])) {
    ...
    // 🛡️ 步驟 1：先去資料庫查這個店家「目前到底還剩幾份」
    $sql_stock = "SELECT IFNULL(SUM(quantity), 0) AS max_stock FROM seller_product WHERE store_name = '$b_store_name'";
    $res_stock = mysqli_query($link, $sql_stock);
    $row_stock = mysqli_fetch_assoc($res_stock);
    $max_stock = (int)$row_stock['max_stock'];

    // 🛡️ 步驟 2：檢查購物車內是不是已經有這家店的東西了
    $sql_check_cart = "SELECT * FROM cart WHERE user_name = '$u_name_escaped' AND store_name = '$b_store_name'";
    ...
    if (mysqli_num_rows($res_check_cart) > 0) {
        ...
        // 🚨 核心防禦：如果總數大於店家的庫存，直接阻斷！
        if ($new_qty > $max_stock) {
            $allow_add = $max_stock - $current_cart_qty; // 算出還能補幾份
            if ($allow_add > 0) {
                echo "<script>alert('⚠️ 加入失敗！您的購物車裡已經有 {$current_cart_qty} 份，但店家目前總共只剩 {$max_stock} 份囉。您最多只能再加 {$allow_add} 份！'); window.history.back();</script>";
            } else {
                echo "<script>alert('⚠️ 購物車內的數量已達店家庫存上限 ({$max_stock} 份) ，無法再追加囉！'); window.history.back();</script>";
            }
            mysqli_close($link);
            exit();
        } else {
            // 數量安全，執行更新
            mysqli_query($link, "UPDATE cart SET quantity = $new_qty WHERE user_name = '$u_name_escaped' AND store_name = '$b_store_name'");
            echo "<script>alert('✅ 成功將「{$b_store_name}」的剩食盲盒放入購物車囉！'); window.location.href='cart.php';</script>";
        }
        
    } else {
        // 購物車裡還沒有，但一樣防呆一下（避免惡意竄改前端數據）
        if ($b_qty > $max_stock) {
            echo "<script>alert('⚠️ 加入失敗！店家庫存僅剩 {$max_stock} 份，無法提供 {$b_qty} 份。'); window.history.back();</script>";
            mysqli_close($link);
            exit();
        } else {
            // 數量安全，執行新增
            mysqli_query($link, "INSERT INTO cart (user_name, store_name, quantity) VALUES ('$u_name_escaped', '$b_store_name', $b_qty)");
            echo "<script>alert('✅ 成功將「{$b_store_name}」的剩食盲盒放入購物車囉！'); window.location.href='cart.php';</script>";
        }
    }
...
<head>
    ...
    <style>
        /* 💡 這裡只保留店家頁面專屬的 CSS，共用的交給 shared_header 處理 */
            ...
            <form method="POST" action="store_detail.php?id=<?php echo urlencode($store_no); ?>">
                ...
                <div class="product-card">
                    ...
                    <div class="product-body">
                        <div class="product-name">今日剩食盲盒</div>
...
<script>
    // 💡 這裡只保留加減數量的專屬 JS，側邊欄已經交給 shared_header
