...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'contact_admin') {
    ...
    // 實務作法：寫入系統通知表或新增客服資料表，這裡我們先寫入 notifications 讓管理者可見
    $admin_content = "【買家客服提問】來自用戶 {$uName} 的訊息： " . $user_msg;
    $sql_insert = "INSERT INTO notifications (user_name, content, created_at) VALUES ('$uName_escaped', '$admin_content', NOW())";
